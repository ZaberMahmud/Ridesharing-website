...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ride_creator_id = $_SESSION['student_id'];

    $newRide = [ 
        'area' => $_POST['pickup_area'],
    ...
    $prefStmt = $conn->query("
        SELECT rp.*, u.Brac_mail, u.Name 
        FROM ride_preferences rp 
        JOIN users u ON rp.student_id = u.student_id 
        WHERE rp.student_id != $ride_creator_id
    ");

    while ($row = $prefStmt->fetch_assoc()) {
        $match = false;

        if (
            $newRide['area'] === $row['area'] &&
            $newRide['date'] >= $row['start_date'] &&
            $newRide['date'] <= $row['end_date']
        ) {
            $timeslots = explode(",", $row['timeslots']);
            $timeslots = array_map('trim', $timeslots);

            if (in_array($newRide['timeslot'], $timeslots)) {
                // Gender preference match
                if ($row['preferred_gender'] === 'Any' || $row['preferred_gender'] === $creator_gender ||$gender === 'Both')  {
                    if ($semesterPref >= $rideSemester) {
                        $match = true;
                    }
                }
            }
        ...
        if ($match) {
            ...
            try {
                ...
                $mail->setFrom('nszarif522@gmail.com', 'RideShare Notifications');
                $mail->addAddress($row['Brac_mail'], $row['Name']);
                $mail->Subject = "🚗 New Ride Matches Your Preference!";
                $mail->Body = "Hello {$row['Name']},\n\n"
                    . "A new ride has just been posted that matches your saved ride preference:\n\n"
                    . "📍 Pickup Area: {$newRide['area']}\n"
                    . "📅 Date: {$newRide['date']}\n"
                    . "🕒 Timeslot: {$newRide['timeslot']}\n\n"
                    . "Visit RideShare now to apply before seats run out!\n\n"
                    . "Thanks,\nRideShare Team";

                $mail->send();
            } catch (Exception $e) {
                echo "❌ Mailer error for {$row['Brac_mail']}: {$mail->ErrorInfo}<br>";
            }
        }
    }
}
